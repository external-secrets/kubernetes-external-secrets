...
describe('Poller', () => {
  ...
  describe('_poll', () => {
    ...
    it('polls secrets', async () => {
      ...
      expect(loggerMock.info.calledWith(`running poll on the secret ${poller.name}`)).to.equal(true)
    ...
    it('logs error if storing secret operation fails', async () => {
      ...
      expect(loggerMock.error.calledWith(`failure while polling the secret ${poller.name}`)).to.equal(true)
    })
  })
  ...
  describe('start', () => {
    ...
    it('starts poller', async () => {
      ...
      expect(loggerMock.info.calledWith(`starting poller on the secret ${poller.name}`)).to.equal(true)
  ...
  describe('stop', () => {
    ...
    it('stops poller', async () => {
      poller.start({ forcePoll: true })
      poller.stop()
      expect(loggerMock.info.calledWith(`stopping poller on the secret ${poller.name}`)).to.equal(true)
      expect(poller._interval).to.equal(null)
    })
  })
})
