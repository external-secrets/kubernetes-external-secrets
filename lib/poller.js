...
class Poller {
  ...
  async _createSecretManifest () {
    ...
    let data = await this._backends[spec.backendType]
    ...
    if (template && typeof template === 'object' && !Array.isArray(template)) {
      ...
      if ("tags" in data) {
        let tagsSpread = JSON.parse(decodedData.tags)
        if (!isEmpty(tagsSpread)) {
          tags = tagsSpread.reduce(((r, c) => Object.assign(r, c)), {})
          var encondedTags = mapValues(tags, value => Buffer.from(value, 'ascii').toString('base64'))
        }
        delete secret.tags 
        data = { ...encondedTags, ...secret}
      }
    }
