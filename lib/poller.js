...
class Poller {
   ...
   */
  async _createSecretManifest (versionSuffix) {
    ...
    const secretManifest = {
      ...
      kind: 'Secret',
      metadata: {
        name: versionSuffix ? `${this._name}-${versionSuffix}` : this._name,
        ownerReferences: [
          this._ownerReference
        ]
      },
   ...
   */
  async poll (versionSuffix) {
    this._logger.info(`running poll on the secret ${this._namespace}/${this._name}`)

    try {
      await this._upsertKubernetesSecret(versionSuffix)
      await this._updateStatus(`SUCCESS${versionSuffix ? `- Version: ${versionSuffix}` : ''}`)
   ...
   */
  async _upsertKubernetesSecret (versionSuffix) {
    const secretName = versionSuffix ? `${this._name}-${versionSuffix}` : this._name
    ...
    if (!verdict.allowed) {
      throw (new Error(`not allowed to fetch secret: ${this._namespace}/${secretName}: ${verdict.reason}`))
    }

    const secretManifest = await this._createSecretManifest(versionSuffix)
    this._logger.info(`upserting secret ${this._namespace}/${secretName}`)
  ...
  _setNextPoll (nextPollIn = this._intervalMilliseconds) {
    ...
    this._timeoutId = setTimeout(this.poll.bind(this), nextPollIn)
