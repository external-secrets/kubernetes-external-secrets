...
class GCPSecretsManagerBackend extends KVBackend {
  ...
  */
   ...
   * @param {string} keyOptions.property - If the backend secret is an object, this is the property name of the value to use.
  ...
  async _get ({ key, keyOptions, specOptions: { projectId } }) {
    ...
    if (keyOptions && keyOptions.property) {
      let parsedValue

      try {
        parsedValue = JSON.parse(secret)
      } catch (e) {
        this._logger.warn(`Failed to JSON.parse value for '${key}',` +
        ' please verify that your secret value is correctly formatted as JSON.' +
        ` To use plain text secret remove the 'property: ${keyOptions.property}'`)
        return
      }

      if (!(keyOptions.property in parsedValue)) {
        throw new Error(`Could not find property ${keyOptions.property} in ${key}`)
      }

      return parsedValue[keyOptions.property]
    }
