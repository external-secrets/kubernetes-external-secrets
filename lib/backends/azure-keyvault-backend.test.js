...
describe('AzureKeyVaultBackend', () => {
  ...
  async function * listSecretsFake () {
    for (const secret of Object.keys(azureSecrets)) {
      yield ({ name: secret })
    }
  }
  ...
  describe('_getByPath', () => {
    it('returns all secrets', async () => {
      const secretsObj = await azureKeyVaultBackend._getByPath({
        path: 'secrets',
        specOptions: {
          keyVaultName: keyVaultName
        }
      })
      const expectedObj = Object.fromEntries(
        Object.entries(azureSecrets).map(([k, v]) => [k, v.value])
      )
      expect(secretsObj).to.eql(expectedObj)
    })
  })
