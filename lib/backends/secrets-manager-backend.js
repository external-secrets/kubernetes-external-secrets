...
class SecretsManagerBackend extends KVBackend {
   ...
   */
  async _get ({ key, specOptions: { roleArn, region, tagImport }, keyOptions: { versionStage = 'AWSCURRENT', versionId = null } }) {
    ...
    if (tagImport) {
      ...
      if("Tags" in tagsResponse) {
        tagsToImport = await tagsResponse.Tags.map(e => {
          if (!([e['Key']] == null) && !(e['Value'] == null)) {
            return {[e['Key'].replace(/[\W]+/g, '-')]: e['Value'].replace(/[\W]+/g, '-')}
          }
          return {}
        })
      }
    ...
    if ('SecretBinary' in data) {
      return {secret: data.SecretBinary, tags : tagsToImport}
    } else if ('SecretString' in data) {
      return {secret: data.SecretString, tags : tagsToImport}
