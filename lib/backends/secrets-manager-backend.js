...
class SecretsManagerBackend extends KVBackend {
   ...
   * @param {string} keyOptions.version - Version stage
   ...
   */
  async _get ({ key, specOptions: { roleArn }, keyOptions: { version = 'AWSCURRENT' } }) {
    ...
    const data = await client
      .getSecretValue({ SecretId: key, VersionStage: version })
      .promise()
