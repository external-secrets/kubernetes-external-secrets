...
describe('GCPSecretManagerBackend', () => {
  ...
  describe('_get', () => {
    beforeEach(() => {
      clientMock.accessSecretVersion = sinon.stub().returns(response)
      gcpSecretManagerBackend._getProjectId = sinon.stub().returns('fakeProjectId')
    })

    it('returns secret property value', async () => {
      const secretPropertyValue = await gcpSecretManagerBackend._get({ key: secretKey })
