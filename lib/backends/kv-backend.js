...
class KVBackend extends AbstractBackend {
   ...
   * @param {string} secretProperties[].version - If the backend supports versioning. For secretsManager backend,
   *   the version is a version stage.
  ...
  _fetchSecretPropertyValues ({ externalData, roleArn }) {
    return Promise.all(externalData.map(async secretProperty => {
      ...
      const value = await this._get({
        secretKey: secretProperty.key,
        secretVersion: secretProperty.version,
        roleArn
      })
