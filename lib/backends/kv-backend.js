...
class KVBackend extends AbstractBackend {
   ...
   */
  constructor ({ logger, objectCache }) {
    super()
    this._logger = logger
  ...
  _fetchDataValues ({ data, specOptions }) {
    return Promise.all(data.map(async dataItem => {
      ...
      const plainOrObjValue = await this._getWithCache({ key, keyOptions, specOptions })
  ...
  _fetchDataFromValues ({ dataFrom, specOptions }) {
    return Promise.all(dataFrom.map(async key => {
      const value = await this._getWithCache({ key, specOptions, keyOptions: {} })
  ...
  async _getWithCache (getOpts) {
    var cachedValue = this.objectCache.get(getOpts)
    if (!cachedValue) {
      var result = await this._get(getOpts)
      this._objectCache.set(getOpts, result)
      return result
    } else {
      return this.objectCache.get(getOpts)
    }
  }
