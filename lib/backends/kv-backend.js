  ...
  _fetchDataValues ({ data, specOptions }) {
    return Promise.all(data.map(async dataItem => {
      ...
      if (shouldParseValue) {
        let parsedValue
        try {
          parsedValue = JSON.parse(value.secret)
        } catch (err) {
      ...
      if (isBinary) {
        ...
        if (typeof value === 'string') {
          ...
          value = Buffer.from(value.secret, 'base64')
        }
      }

      if (singleParameterKey) {
        // Not path, return as is
        response = { [name]: value.secret, tags: value.tags}
        //response = { [name]: value }
      } else {
        // Returning dict with path keys and values
        let tagsMerge;
        for (const records in value) {
          response[records.secret] = value[records.secret]
          tagsMerge = { ...tagsMerge, ...records.tags}
        }
        response.tags = value.tagsMerge
      }
      return response
    }))
  }
  ...
  _fetchDataFromValues ({ dataFrom, specOptions }) {
    return Promise.all(dataFrom.map(async key => {
      ...
      try {
        ...
        return {...JSON.parse(value.secret), tags: value.tags}
      } catch (err) {
        this._logger.warn(`Failed to JSON.parse value for '${key}',` +
          ' please verify that your secret value is correctly formatted as JSON.')
      }
