...
describe('CustomResourceManager', () => {
  ...
  describe('_getResource', () => {
    it('gets custom resource', async () => {
      ...
      expect(v1beta1Mock.customresourcedefinitions
        .calledWith(fakeResourceName)).to.be.true;
      expect(v1beta1Mock.customresourcedefinitions.get.called).to.be.true;
    });
  });
  ...
  describe('upsertResource', () => {
    ...
    it('creates custom resource', async () => {
      ...
      expect(kubeClientMock.addCustomResourceDefinition.calledWith(fakeCustomResourceManifest))
        .to.be.true;
      expect(customResourceManager._createResource.calledWith({
        customResourceManifest: fakeCustomResourceManifest
      })).to.be.true;
      expect(customResourceManager._getResource.called).to.be.false;
      expect(customResourceManager._updateResource.called).to.be.false;
    });
    ...
    it('updates custom resource', async () => {
      ...
      expect(customResourceManager._sleep.called).to.be.false;
    });
  });
});
